    ...
    public class AmsJournalController : ControllerBase
    {
        ...
        private DateTime GetJrnMaxUTC_SQL(MaxUTCBuffer maxUTCBuffer)
        {
            ...
            using (SqlConnection sqlConn = new SqlConnection(connectionString))
            {
                ...
                SqlCommand sqlCmd = new SqlCommand($"SELECT MAX(UTC) FROM {AMSOperator.TABLE_JRN[maxUTCBuffer.tblIdx]}", sqlConn);
        ...
        private void AddJournal_SQL(string DBName, DateTime UTC, List<short> chIdList, List<short> chValList)
        {
            ...
            using (SqlConnection sqlConn = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    for (int tblIndex = 0; tblIndex < AMSOperator.TABLE_JRN.Length; tblIndex++)
                    {
                        int minChId = AMSSysInfo.CH_COUNT / AMSOperator.TABLE_JRN.Length * tblIndex;
                        int maxChId = AMSSysInfo.CH_COUNT / AMSOperator.TABLE_JRN.Length * (tblIndex + 1) - 1;
                        ...
                        if (sqlParts.ContainValues)
                        {
                            ...
                            sqlCmd.CommandText = $"IF NOT EXISTS (SELECT UTC FROM {AMSOperator.TABLE_JRN[tblIndex]} WHERE UTC=@UTC) " +
                                                 $"INSERT INTO {AMSOperator.TABLE_JRN[tblIndex]} ({sqlParts.ColumnNames}) VALUES ({sqlParts.Values})";
