    ...
    public class AmsDTAController : ControllerBase
    {
        ...
        private Dictionary<int, List<DateTime>> GetDTAMaxUTC_SQL(MaxUTCBuffer maxUtCBuffer)
        {
            ...
            for (int i = 0; i < maxUtCBuffer.DTAFileIdxList.Count; i++)
            {
                ...
                strSqlCmd += (strSqlCmd.Length > 0 ? " UNION ALL " : "") +
                             $"SELECT {fileIdx},MAX(UTC) FROM {AMSOperator.TABLE_DTA[DTATblIdx]} WHERE C{fileIdx * 2 + 1} IS NOT NULL " +
                             $"UNION ALL SELECT {fileIdx}, MAX(UTC) FROM {AMSOperator.TABLE_DTA[DTATblIdx]} WHERE C{fileIdx * 2 + 2} IS NOT NULL";
            }
        ...
        private void AddWaveRecord_SQL(string DBName, int fileIdx, int flag, DateTime UTC, byte[][] data)
        {
            ...
            using (SqlConnection sqlConn = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.CommandText = $"IF NOT EXISTS (SELECT UTC FROM {AMSOperator.TABLE_DTA[fileIdx / 12]} WHERE UTC=@UTC) " +
                                     $"INSERT INTO {AMSOperator.TABLE_DTA[fileIdx / 12]} (UTC) VALUES (@UTC)";
                sqlCmd.Parameters.AddWithValue("@UTC", UTC);
                sqlCmd.ExecuteNonQuery();

                sqlCmd.Parameters.Clear();
                sqlCmd.CommandText = $"IF NOT EXISTS (SELECT UTC FROM {AMSOperator.TABLE_DTA[fileIdx / 12]} WHERE UTC=@UTC {where}) " +
                                     $"UPDATE {AMSOperator.TABLE_DTA[fileIdx / 12]} SET {update} WHERE UTC=@UTC";
                sqlCmd.Parameters.AddWithValue("@UTC", UTC);
                if ((flag & 1) == 1)
                {
